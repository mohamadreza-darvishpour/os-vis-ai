...
def simulate_rr(
    ...
    while True:
        ...
        if ready_queue:
            ...
            # Add dispatcher latency for context switch if necessary
            if gantt_chart and gantt_chart[-1][0] != f"P{current + 1}":
            ...
            if remaining_burst[current] == 0:
                ...
                if burst_index[current] < len(process_burst_list[current]):
                    ...
                    io_end_time = time + process_burst_list[current][burst_index[current]]
                    io_queue.append((current, io_end_time))
                    burst_index[current] += 1
                    if burst_index[current] < len(process_burst_list[current]):
        ...
        # Remove completed processes from the simulation
        for i in range(n):
            if burst_index[i] >= len(process_burst_list[i]) and completion_time[i] == -1:
        ...
        if not ready_queue and io_queue:
            next_io_end_time = min(io_end_time for _, io_end_time in io_queue)
            gantt_chart.append(("Idle", next_io_end_time - time))
            time = next_io_end_time
